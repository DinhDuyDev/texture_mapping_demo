...
def raycast_new(screen_width, screen_height, resolution, x, y, z, dirX, dirY, planeX, planeY, fov, maph:list[list[int]]):
    ...
    for i in range(resolution):
        ...
        height = (RAYCAST_SIZE_SCALE / (perpWallDist * height_scale))

        # texture surfae
        texture_surface = pygame.transform.scale(texture_strips[texX], (column_width, height))

        strip = screen_elements.ScreenElement((resolution-i) * column_width, screen_height/2-height/2, perpWallDist, height+0.1, texture_surface)
    ...
    for sprite in worldsprite.WorldSprite.all_sprites:
        ...
        sprite_x = __x - posX
        sprite_y = __y - posY
        distToSprite = utilityfuncs.point_distance(x, y, sprite.x, sprite.y)
        ...
        transformY = invDet * (-planeY * sprite_x + planeX * sprite_y) # depth inside the screen

        sprite_screen_x = int((resolution / 2) * (1 + transformX / transformY))
        sprite_height = min(abs(int(screen_height / transformY)), 1280)
        sprite_screen_y = (screen_height/2) - sprite_height/2
